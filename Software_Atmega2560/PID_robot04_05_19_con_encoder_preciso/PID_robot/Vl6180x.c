...
{
	if (startI2C()) {stop();Serial_print("start");}									//mi restituisce 1 se lo start NON è avvenuto correttamente
	if (sendI2C(slave|TW_WRITE)) {stop();Serial_print("slave");}				////mi restituisce 1 se la trasmissione NON è avvenuta correttamente
...
// for details.
void startRangeContinuous(uint16_t period)//periodo inversamente proporzionale alla velocià
{
	period = constrain(period, 10/*50*/, 100);//per variare velocità di lettura	-> rischio di blocco maggiore se si aumenta la velocità
